...
export async function resolveDeno(id: string, cwd: string): Promise<DenoResolveResult | null> {
	...
	const output = await new Promise<string | null>((resolve, reject) =>
		execFile(DENO_BINARY, ['info', '--json', id], { cwd }, (error, stdout) => {
			if (error)
				if (String(error).includes('Integrity check failed')) reject(error);
				else resolve(null);
			else resolve(stdout);
		})
	);
...
export function toDenoSpecifier(loader: DenoMediaType, id: string, resolved: string): DenoSpecifierName {
...
export function parseDenoSpecifier(spec: DenoSpecifierName): {
	...
	const [_, loader, id, posixPath] = spec.split('::') as [string, string, DenoMediaType, string];
